...
export function Dashboard({ theme, setTheme }) {
  ...
  const loadData = useCallback(async (silent = false) => {
    // Ensure we don't treat event objects as 'not silent' (which would skip loading indicator)
    ...
    try {
      ...
      if (!isSilent) {
        if (hasError) showToast("Some data failed to refresh", "error");
        else showToast("Data refreshed successfully", "success");
      }
    } catch (e) {
      if (!isSilent) showToast("Refresh failed: " + e.message, "error");
    } finally {
      setLoading(false);
  ...
  const page = () => {
    ...
    const props = { mobs, setMobs, users, setUsers, suppliers, setSuppliers, livestock, chartData, toast: showToast, currentUser, loading };
    switch (active) {
  ...
  return (
    <div className="dashboard-layout" style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--color-background-tertiary)", overflow: "hidden" }}>
        <TopBar active={active} onRefresh={loadData} loading={loading} theme={theme} setTheme={setTheme} setIsCollapsed={setIsCollapsed} isCollapsed={isCollapsed} />
