...
export function OverviewPage({ mobs, chartData, loading }) {
  ...
  const exportData = useMemo(() => {
    return mobs.map(m => ({
      "MOB No.": m.mob_number || m.id,
      "Supplier": m.supplier?.name || "N/A",
      "Status": m.mob_status,
      "Livestock Count": getMobLivestockCount(m),
      "Total Weight (KG)": Number(m.total_weight || 0).toFixed(2),
      "Received Date": m.received_date ? new Date(m.received_date).toLocaleDateString('en-GB') : "N/A",
      "Email Status": m.email_status || "PENDING"
    }));
  }, [mobs]);

  const exportOptions = useMemo(() => ({
    title: "KMC Livestock Operations Overview",
    sheetName: "MOB Records",
    summaryData: {
      "Total MOBs": total,
      "Total Livestock Headcount": livestockCount,
      "Total Weight Managed (KG)": totalWeight.toFixed(2),
      "Average Animal Weight (KG)": avgWeight.toFixed(2),
      "Open MOBs": open,
      "Email Delivery Failures": failed
    },
    analysisSheets: [
      {
        title: "Monthly Intake Analysis",
        sheetName: "Monthly Trends",
        data: intakeData.map(d => ({ "Month": d.month, "Livestock Count": d.livestock })),
        headers: ["Month", "Livestock Count"]
      },
      {
        title: "Species Distribution",
        sheetName: "Species Analysis",
        data: speciesCountData.map(d => ({ "Species": d.name, "Count": d.value })),
        headers: ["Species", "Count"]
      }
    ]
  }), [total, livestockCount, totalWeight, avgWeight, open, failed, intakeData, speciesCountData]);
