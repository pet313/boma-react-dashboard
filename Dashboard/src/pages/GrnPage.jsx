...
export function GrnPage({ mobs, setMobs, toast, currentUser }) {
  ...
  const exportOptions = useMemo(() => ({
    ...
    summaryData: {
      "Total Closed MOBs": mobs.filter(m => m.mob_status === "CLOSED").length,
      "Email Success Rate": `${mobs.length ? Math.round(mobs.filter(m => m.email_status === "SENT").length / mobs.length * 100) : 0}%`,
      "Pending Deliveries": mobs.filter(m => m.email_status === "PENDING").length,
      "Failed Deliveries": mobs.filter(m => m.email_status === "FAILED").length
    }
  }), [mobs]);
