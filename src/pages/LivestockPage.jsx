...
export function LivestockPage({ livestock, chartData }) {
  ...
  const dateFiltered = useMemo(() => {
    if (!startDate && !endDate) return livestock;
  ...
  }, [livestock, startDate, endDate]);

  const total  = dateFiltered.length;
  const manual = dateFiltered.filter(a => a.manual_entry).length;
  const avgW   = total ? (dateFiltered.reduce((s, a) => s + Number(a.weight || 0), 0) / total) : 0;
  ...
  }), [total, manual, avgW, genderData, startDate, endDate]);
