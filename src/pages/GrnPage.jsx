...
export function GrnPage({ mobs, setMobs, toast, currentUser }) {
  ...
  const exportData = useMemo(() => {
    return filtered.map(m => ({
      ...
      "GRN Number": m.grn_number || "PENDING",
  ...
  const exportOptions = useMemo(() => ({
    title: "KMC GRN & Email Delivery Report",
    sheetName: "GRN Records",
    summaryData: {
      "Report Period": `${startDate || 'Start'} to ${endDate || 'Present'}`,
      "Total Closed MOBs": dateFiltered.filter(m => m.mob_status === "CLOSED").length,
      "Email Success Rate": `${dateFiltered.length ? Math.round(dateFiltered.filter(m => m.email_status === "SENT").length / dateFiltered.length * 100) : 0}%`,
      "Pending Deliveries": dateFiltered.filter(m => m.email_status === "PENDING").length,
      "Failed Deliveries": dateFiltered.filter(m => m.email_status === "FAILED").length
    }
  ...
  const closedMobs = dateFiltered.filter(m => m.mob_status === "CLOSED");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        <ExportButton
          data={exportData}
          fileName="KMC_GRN_Delivery_Report"
          options={exportOptions}
          label="Export GRN Report"
        />
      </div>
